...
def parse_args():
    ...
    parser.add_argument("--model_type", help="cnn/wrn-40-2/wrn-28-10/preactresnet/resnet34",
                        type=str, default="wrn-28-10",
                        choices=["cnn", "wrn-40-2", "wrn-28-10", "preactresnet", "resnet34"])
    ...
    parser.add_argument("--normalize", help="Normalize training data inside the model", type=int, default=1,
                        choices=[0, 1])
    # Only valid if restarts = 1: #0 -> Always start from Zero, #1-> Always start with 1, #2-> Start from 0/rand with
    # prob = 1/2
    ...
    parser.add_argument("--imagenet_architecture", help="Imagenet Architecture", type=str, default="wrn",
                        choices=["wrn", "alexnet", "inception"])

    # Threat models
    parser.add_argument("--mode", help="Various threat models", type=str, default='teacher',
                        choices=['zero-shot', 'fine-tune', 'extract-label', 'extract-logit', 'distillation',
                                 'teacher', 'independent', 'pre-act-18'])
    ...
    # LR
    parser.add_argument("--lr_mode", help="Step wise or Cyclic", type=int, default=4)
    ...
    parser.add_argument("--feature_type", help="Feature type for generation", type=str, default='mingd',
                        choices=['pgd', 'topgd', 'mingd', 'rand'])

    parser.add_argument("--regressor_embed", help="Victim Embeddings for training regressor", type=int, default=0,
                        choices=[0, 1])
...
def add_config(args):
    data = yaml.load(open(args.config_file, 'r'), Loader=yaml.FullLoader)
    args_dict = args.__dict__
    for key, value in data.items():
        if '--' + key in sys.argv and args_dict[key] is not None:  # Giving higher priority to arguments passed in cli
            continue

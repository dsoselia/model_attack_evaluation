...
train_loader_1 = torch.utils.data.DataLoader(train_1, batch_size=batch_size, shuffle=True, num_workers=num_workers)
train_loader_2 = torch.utils.data.DataLoader(train_2, batch_size=batch_size, shuffle=True, num_workers=num_workers)
val_loader_1 = torch.utils.data.DataLoader(val_1, batch_size=batch_size, shuffle=False, num_workers=num_workers)
val_loader_2 = torch.utils.data.DataLoader(val_2, batch_size=batch_size, shuffle=False, num_workers=num_workers)
stealing_loader = torch.utils.data.DataLoader(stealing_set, batch_size=batch_size, shuffle=False, num_workers=num_workers)
...
# %%
def train_model(model, train_loader, val_loader, epochs=100, lr=0.001, momentum=0.9):
...
def get_random_walk_diff_vector(model, x_0, y, step_size=0.01, random_direction_matrix=None):
    with torch.no_grad():
...
random_directions_n = 20
for i in range(random_directions_n):
    random_direction_matrix_list.append(torch.randn(train_set[0].shape))
    ...
    for inputs, labels in dataset:
        for img, label in zip(inputs, labels):
            ...
            for random_direction_matrix in random_direction_matrix_list:
                steps_taken = get_random_walk_diff_vector(model, img, label, random_direction_matrix=random_direction_matrix)
                k.append(steps_taken)
            distances.append(k)
            distances_labels.append(label)
    return distances, distances_labels

distances, distances_labels = get_distances_for_model(model_1, train_loader_1, random_direction_matrix_list, 0)
# concatinate val_loader_1 distances 
val_distances, val_distances_labels = get_distances_for_model(model_1, val_loader_1, random_direction_matrix_list, 1)
...
transform_distances = transforms.Compose([ transforms.ToTensor(), transforms.Normalize((0.5,), (0.5,))])
...
train_distances, val_distances = torch.utils.data.random_split(distances_dataset, [int(len(distances_dataset) * 0.8), len(distances_dataset) - int(len(distances_dataset) * 0.8)])
train_distances_loader = torch.utils.data.DataLoader(train_distances, batch_size=4, shuffle=True, num_workers=num_workers)
val_distances_loader = torch.utils.data.DataLoader(val_distances, batch_size=4, shuffle=False, num_workers=num_workers)
...
optimizer_binary = torch.optim.SGD(binary_classifier.parameters(), lr=0.001, momentum=0.9)
binary_epochs = 10
...
stolen_distances, stolen_distances_labels = get_distances_for_model(stolen_model, train_loader_1, random_direction_matrix_list, 0)
...
stolen_distances_dataset = torch.utils.data.TensorDataset(stolen_distances, stolen_distances_labels)

stolen_distances_loader = torch.utils.data.DataLoader(stolen_distances_dataset, batch_size=4, shuffle=False, num_workers=num_workers)
...
def test_if_stolen(model, dataloader, binary_classifier, random_direction_matrix_list):
    stolen_distances, stolen_distances_labels = get_distances_for_model(model, dataloader, random_direction_matrix_list, 0)
    stolen_distances = torch.tensor(stolen_distances)
    stolen_distances = transform_distances(stolen_distances)
    stolen_distances_labels = torch.tensor(stolen_distances_labels)
    stolen_distances_dataset = torch.utils.data.TensorDataset(stolen_distances, stolen_distances_labels)
...
stolen_model_score = test_if_stolen(stolen_model, train_loader_1, binary_classifier, random_direction_matrix_list)

print("stolen model score: ", stolen_model_score)
# call on model_2 and train_1 loader 

model_2_score = test_if_stolen(model_2, train_loader_1, binary_classifier, random_direction_matrix_list)
